...
class World:
    ...
    def can_go(self, here, there):
        return (0 <= there.u < self.width and 0 <= there.v < self.height
                ...
                and (self.elevation(here) <= chr(ord(self.elevation(there)) + 1)))
...
class Filler:
    ...
    def fill_all(self):
        self.looking_at = set([self.state.target])
        self.world.put(self.state.target, 1)
        ...
        for distance in range(1, self.world.height * self.world.width):
            ...
            for here in visited:
                if self.world.elevation(here) == 'a':
                    print(f"{distance=}")
                    return
